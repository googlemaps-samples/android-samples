...
object GrievanceReportExporter {
    ...
    fun generateAndSaveReport(
        ...
        if (grievances.isNotEmpty()) {
            ...
            grievances.forEachIndexed { index, item ->
                ...
                md.append("- **Target FQCN**: `${item.sampleId}`\n")
                md.append("- **Framework**: `${item.framework}`\n")
        ...
        md.append("| Status | Sample Title | Target (FQCN) | Framework | Category | Notes |\n")
        md.append("|---|---|---|---|---|---|\n")
        evaluations.forEach { item ->
            ...
            md.append("| $statusBadge | ${item.sampleTitle} | `${item.sampleId}` | ${item.framework} | ${item.category} | $notePreview |\n")
        }
        md.append("\n---\n*Report generated by GMP Sample Reviewer Mode.*\n")

        val fileName = "evaluation_report_${fileSuffix}.md"
        val targetDir = File(context.getExternalFilesDir("reports") ?: context.filesDir, "").apply {
            mkdirs()
        }
    ...
    fun createShareIntent(context: Context, file: File): Intent {
        ...
        val reportContent = try {
            ...
            "GMP Samples Evaluation Report (${file.name})"
        }
        ...
        return Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "GMP Samples - Evaluation Report (${file.name})")
