...
def generate_html_report(
    ...
    css_content = (scripts_dir / "report_style.css").read_text(encoding="utf-8") if (scripts_dir / "report_style.css").exists() else ""
    js_template = (scripts_dir / "review_dashboard.js").read_text(encoding="utf-8") if (scripts_dir / "review_dashboard.js").exists() else ""
    embedded_json = json.dumps(results, indent=2).replace("</script>", r"<\/script>")
    js_content = js_template.replace("__RUN_ID__", run_id).replace("__RAW_RESULTS__", embedded_json)
...
def main():
    ...
    if args.run_dir:
        ...
        runs = sorted([d for d in (root_dir / "eval_runs").iterdir() if d.is_dir() and (d.name.startswith("run_") or d.name.startswith("spelunk_"))])
        if not runs:
    ...
    print(f"🖥️ Local URL: http://dirtdog.c.googlers.com:{args.port} (when served)")
    print("=" * 75)

    if args.serve:
        print(f"\n🚀 Review server listening on http://dirtdog.c.googlers.com:{args.port}...")
